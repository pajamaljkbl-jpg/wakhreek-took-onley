...
export default function Boutique() {
  ...
  const [product, setProduct] = useState({ name: '', description: '', category: 'Produits naturels', price: '', stock: '' });
  const [productImage, setProductImage] = useState(null);

  useEffect(() => { fetch('/api/public-config').then((r) => r.json()).then(setConfig).catch(() => {}); }, []);
  ...
  async function submitSubscription() {
    if (!proof) return setMessage('Ajoute la capture du paiement Wave.');
  ...
  return <main style={{ minHeight: '100vh', background: '#f5f7fa', padding: 20, fontFamily: 'Inter,system-ui,sans-serif' }}>
    <section style={{ maxWidth: 560, margin: '20px auto', background: 'white', padding: 24, borderRadius: 22 }}>
      <a href="/" style={{ color: BLUE }}>← Retour</a><h1>Créer ma boutique</h1>
      {!shop ? <form onSubmit={createShop}>
        ...
        </form><hr style={{ margin: '24px 0', border: 0, borderTop: '1px solid #eee' }} />
        <h2>Abonnement : 6 000 F</h2>
        ...
        {!config.adminWaveQrUrl && !config.adminWaveNumber && <p style={{ color: '#d00' }}>Le compte Wave administrateur n’est pas encore configuré.</p>}
